...
def run_supervisor(base: Path, ollama: str, model: str, prompt_rel: str, task: str, temperature: float) -> str:
    sys_prompt = load_prompt(base, prompt_rel)
    messages = [
        {"role": "system", "content": sys_prompt},
        {"role": "user", "content": f"BASE_DIR={base}\nTASK:\n{task}\n\nReturn: plan steps + acceptance criteria + verification commands."},
    ]
    return ollama_chat(ollama, model, messages, temperature=temperature)

def phase_user_content(base: Path, apply: bool, task: str, plan: Optional[str]) -> str:
    uc = (
        f"BASE_DIR={base}\n"
        f"APPLY={apply}\n"
        f"TOOLS={tool_help()}\n\n"
...
def main() -> int:
    ...
    ap.add_argument("--pipeline", default="build", help="Pipeline name (build|harden|docs|...)")
    ap.add_argument("--ollama", default=DEFAULT_OLLAMA)
    ap.add_argument("--model", default=DEFAULT_MODEL)
    ap.add_argument("--apply", action="store_true", help="Allow write/patch phases that permit it")
    ...
    for idx, ph in enumerate(phases, start=1):
        ...
        prompt_rel = str(ph.get("prompt", "apps/agent/prompts/kryptos_builder.system.md"))
        ...
        if role == "supervisor":
            plan = run_supervisor(base, args.ollama, args.model, prompt_rel, task, args.temperature)
            transcript.append(f"\n=== SUPERVISOR PLAN ===\n{plan}\n")
            continue
        ...
        if tool_loop:
            ...
            out = ollama_chat(args.ollama, args.model, messages, temperature=args.temperature)
            transcript.append(f"\n=== {role.upper()} (one-shot) ===\n{out}\n")
