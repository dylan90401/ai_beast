#!/usr/bin/env python3
import os, sys, json, subprocess, secrets
...
def run_beast(key):
  ...
  return 200, {"ok": p.returncode==0, "returncode": p.returncode, "stdout": p.stdout[-20000:], "stderr": p.stderr[-20000:], "cmd": args}
...
def main():
  ...
  try:
    cfg=load_env_json()
    if cfg.get("PORT_DASHBOARD"): port=int(cfg["PORT_DASHBOARD"])
  except Exception:
    pass
